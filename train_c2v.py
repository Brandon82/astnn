...
import time
import numpy as np
from model_c2v import BatchProgramClassifier
...
if __name__ == '__main__':
    ...
    root = 'data/'
    train_data = pd.read_pickle('./data/split_data/train/code2vec.pkl')
    val_data = pd.read_pickle('./data/split_data/dev/code2vec.pkl')
    test_data = pd.read_pickle('./data/split_data/test/code2vec.pkl')
    ...
    model = BatchProgramClassifier(INPUT_DIM, HIDDEN_DIM, ENCODE_DIM, LABELS, BATCH_SIZE, USE_GPU)
    if USE_GPU:
        model.cuda()
